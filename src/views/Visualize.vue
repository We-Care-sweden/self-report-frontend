      ...
      <section class="section-shaped my-0">
        ...
        <div class="container shape-container d-flex">
          <div class="col text-white">
            ...
            <base-button
              v-for="layerDefinition in layersDefinifion"
            ...
            <p v-if="lastUpdate">
              <small>{{ $t('visualize.lastUpdate') }} {{ lastUpdate.toLocaleString() }}</small>
            </p>
  ...
  methods: {
    ...
    loadLeaflet: async function () {
      _map = L.map('leaflet-map', {
        ...
        [process.env.VUE_APP_VISU_MAP_CENTER_LATITUDE, process.env.VUE_APP_VISU_MAP_CENTER_LONGITUDE],
        process.env.VUE_APP_VISU_MAP_ZOOM_LEVEL
      )

      _tileLayers.push(
        L.tileLayer(this.mapBaseLayerUrl, {
      ...
      for (const entry of _data) {
        ...
        for (const layerDefinition of this.layersDefinifion) {
          try {
            ...
            layerDefinition.data.markers.push(
              L.circle(geocoding.coordinates, {
                weight: 0,
